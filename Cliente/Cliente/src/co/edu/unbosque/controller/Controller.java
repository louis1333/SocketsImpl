...
public class Controller implements ActionListener {
	...
	private Cliente server;
	public Controller() {
		vp = new VentanaPrincipal();
		adv = new Ventanas_AD();
		vp.setVisible(true);
		server = new Cliente("127.0.0.1",5000);
		oyentes();
		run();
	}
			...
			if (verificar() == true) {
				...
				if (verificar_num(nombre) == true | (verificar_num(apellido) | (verificar_num(cargo)))) {
					...
					if (verificar_solonum(edadst)== true && verificar_solonum(cedulast)== true) {
						...
						server.enviarDatos(nombre, apellido, edad, cedula, cargo);
						adv.mostrarinfo("Usuario Agregado exitosamente, finalizando");
						System.exit(0);
